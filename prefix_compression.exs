...
defmodule Solution do
  ...
  def msolve([l1, l2]) do
    prefix = Enum.zip(l1, l2) |> Enum.take_while(fn ({x1, x2}) -> x1 == x2 end)
    p = prefix |> Enum.map(fn ({x1, _}) -> x1 end) |> List.to_string
    IO.puts(Integer.to_string(String.length(p)) <> " " <> p)

    solve([l1, l2])
  end

  def main() do
    lines = String.strip(IO.read(:stdio, :all))
    lines |> String.split("\n") |> Enum.map(&(String.to_charlist(&1))) |> msolve
  end
end
Solution.main()
